...
class BasicAgentRuntime:
    ...
    def emit(self, typ, agent_id, payload=None):
        self.recorder.record(TraceEvent(
            ...
            mission_run_id=self.run_id,
            agent_run_id=agent_id,
            sequence=len(self.recorder.for_run(self.run_id)) + 1,
            event_type=typ,
            payload=payload or {},
        ))

    def _append_assistant_call(self, state: AgentState, call: ToolCall) -> None:
    ...
    def _execute_tool(self, agent_run_id: UUID, state: AgentState, call: ToolCall, append_assistant: bool = True) -> None:
        ...
        if append_assistant:
            self._append_assistant_call(state, call)
        state.messages.append({
            "role": "tool",
        ...
        if result.success and call.name == "edit_file" and self.checkpoint:
            ...
            payload = {"reason": "successful_edit_file"}
            if checkpoint_id:
                payload["checkpoint_id"] = str(checkpoint_id)
            self.emit("checkpoint_created", agent_run_id, payload)

    def continue_approved(self, agent_run_id: UUID, state: AgentState, approval: PendingApproval) -> AgentState:
        if approval.status != ApprovalStatus.APPROVED:
            raise ValueError("approval is not approved")
        if approval.run_id != self.run_id or approval.agent_role != state.role:
            raise ValueError("approval does not belong to agent run")
        if state.pending_approval_id != approval.approval_id:
            raise ValueError("approval does not match pending agent state")
        snapshot = approval.tool_call
        if state.pending_tool_call:
            pending = ToolCallSnapshot.model_validate(state.pending_tool_call)
            if pending != snapshot:
                raise ValueError("pending tool call does not match approval")
        recomputed = ToolCallSnapshot.from_parts(snapshot.tool_name, snapshot.agent_role, snapshot.arguments, snapshot.call_id)
        if recomputed.tool_name != snapshot.tool_name or recomputed.agent_role != snapshot.agent_role or recomputed.arguments_digest != snapshot.arguments_digest:
            raise ValueError("tool call snapshot digest mismatch")
        if not any(
            message.get("role") == "assistant"
            and any(
                call.get("id") == snapshot.call_id
                and call.get("function", {}).get("name") == snapshot.tool_name
                for call in message.get("tool_calls", [])
            )
            for message in state.messages
        ):
            self._append_assistant_call(state, ToolCall(name=snapshot.tool_name, arguments=snapshot.arguments, call_id=snapshot.call_id or None))
        ...
        while not state.finished:
            ...
            try:
                response = self.provider.complete(ModelRequest(
                    ...
                    tools=[TOOL_SPECS[n] for n in state.allowed_tools],
                    role=state.role.value,
                    expected_output=request_expected_output,
                    response_schema=response_schema,
                    metadata={"agent_run_id": str(agent_run_id)},
                ))
            except ProviderError as error:
                self.emit("runtime_error", agent_run_id, {"category": "provider_error", "provider_error_type": error.category, "metadata": error.metadata})
                state.finished = True
                return state
            ...
            if response.kind == "tool" and response.tool_call:
                if state.allowed_tools and response.tool_call.name not in state.allowed_tools:
                    self.emit("validation_error", agent_run_id, {"category": "unauthorized_tool", "tool": response.tool_call.name})
                    state.finished = True
                    return state
                try:
                ...
                try:
                    ...
                    state.handoffs.update(response.output)
                    state.finished = True
                except Exception as error:
                    self.emit("validation_error", agent_run_id, {"category": "structured_output", "schema": state.expected_output or "unknown", "error": str(error)[:160]})
                    state.finished = True
        self.emit("agent_finished", agent_run_id)
        return state
