...
class BasicMissionOrchestrator:
    ...
    def run(self, mission_id: UUID, manifest: ExecutionManifest, fixture: Path, workspace_root: Path, mission_context: dict | None = None) -> MissionRunResult:
        ...
        def execute(role, skills, handoff):
            ...
            aid=uuid4(); state=AgentState(mission_id=mission_id,mission_run_id=run_id,agent_run_id=aid,role=role,level=Level.SENIOR,profile=SkillProfile(name=role.value,skills=skills),messages=[{'role':'user','content':str(mission_context)}] if mission_context else [],handoffs=handoff,allowed_tools=permissions[role],expected_output=expected[role])
            def checkpoint(s):
        ...
        def finish(status, final_qa):
            events=self.recorder.for_run(run_id); self.recorder.record(TraceEvent(mission_id=mission_id,mission_run_id=run_id,sequence=len(events)+1,event_type="mission_finished",payload={"status":status})); events=self.recorder.for_run(run_id)
            changed=[e.payload.get("arguments",{}).get("path") for e in events if e.event_type=="tool_call" and e.payload.get("name")=="edit_file"]
            return MissionRunResult(mission_run_id=run_id,mission_id=mission_id,status=status,retry_count=retry,execution_manifest=manifest,pm_agent_run_id=pm_id,developer_agent_run_ids=dev_ids,qa_agent_run_ids=qa_ids,final_qa_result=final_qa.model_dump(),changed_files=[x for x in changed if x],workspace_reference=str(workspace),tool_call_count=sum(e.event_type=="tool_call" for e in events),event_count=len(events),checkpoint_ids=checkpoints)
        pm_id,pm_state=execute(Role.PM,("common/tool_usage.md","common/handoff.md","roles/pm/SKILL.md"),{})
        if not {"mission_summary","developer_task"} <= pm_state.handoffs.keys(): return finish("FAILED",QAResult(status="failed",issues=["PM runtime did not produce a handoff"]))
        ...
        while True:
            dev_id,dev_state=execute(Role.DEVELOPER,("common/tool_usage.md","common/handoff.md","roles/developer/SKILL.md"),{"pm":pm.model_dump(),"qa_feedback":feedback,"retry":retry}); dev_ids.append(dev_id)
            if not {"status","summary"} <= dev_state.handoffs.keys(): return finish("FAILED",QAResult(status="failed",issues=["Developer runtime did not produce a handoff"]))
            ...
            qa_id,qa_state=execute(Role.QA,("common/tool_usage.md","common/handoff.md","roles/qa/SKILL.md"),{"developer":dev.model_dump()}); qa_ids.append(qa_id)
            if "status" not in qa_state.handoffs: return finish("FAILED",QAResult(status="failed",issues=["QA runtime did not produce a result"]))
