...
from .api.schemas import MissionCreate, MissionResponse, RunResponse, EventResponse
...
def run_response(r): return RunResponse(run_id=r.mission_run_id,mission_id=r.mission_id,status=r.status,retry_count=r.retry_count,changed_files=r.changed_files,tool_call_count=r.tool_call_count,event_count=r.event_count,workspace_ref=r.workspace_reference)
@app.post('/missions/{mission_id}/runs', response_model=RunResponse)
def start_run(mission_id: UUID):
    m=missions.get(mission_id)
    if not m: raise HTTPException(404,'mission not found')
    return run_response(runs.start(m))
