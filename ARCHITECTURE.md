...
Operational identifiers are UUIDs; reusable benchmark suites use a human-readable `suite_id` plus positive integer `version`. Domain records use Pydantic models or frozen dataclasses where practical.
...
- **Experiment / ExperimentCell**: a sealed case × frozen model × 0-based repetition specification and its durable one-cell-to-one-Mission/Run mapping. Experiments may retain a `BenchmarkSuiteProvenance` (`suite_id`, explicit `version`, and `digest`) while always owning their ordered executable case snapshot. Cells retain deterministic identity, canonical order, workspace seed reference, and Run IDs; Run status remains authoritative.
- **ExperimentAnalytics**: a read-only, derived-on-read comparison of only the Runs mapped to one Experiment. It exposes overall, frozen-model, and case aggregates plus valid/incomplete/invalid mapping integrity; it has no winner, rank, score, or recommendation.
- **BenchmarkSuite / BenchmarkCaseSpec**: a reusable, provider/model/runtime-neutral ordered workload. A suite version is `DRAFT` until published, then its cases and execution-relevant SHA-256 `spec_digest` are immutable. `BenchmarkCaseSpec` reuses `ExperimentCase` validation and adds optional description metadata; suite versions are independent from Experiment sealing.
...
The adapter stores missions, run results, append-only trace events, checkpoint state, workspace-snapshot metadata, approvals, experiment specifications, experiment-cell mappings, and benchmark suite definitions. Analytics are derived on read and have no persistence table or cache. Benchmark suites use one JSON spec row per `(suite_id, version)` with a composite primary key; no case table or pickle storage is introduced. UUIDs and datetimes use explicit JSON/primitive serialization; `PRAGMA user_version` records schema version 4. Run finalization writes the result and its events in one short transaction, while checkpoint, workspace, approval, experiment, and benchmark-suite metadata are committed at their safe boundaries. Experiment cell, Mission, and Run IDs are deterministic and persisted before execution so re-entry cannot create a second identity; the existing adapter does not require a cross-table transaction. Approval decision state and its audit event use one local SQLite transaction; filesystem side effects remain outside that transaction.
...
- `POST /benchmark-suites`, `GET /benchmark-suites`
- `GET /benchmark-suites/{suite_id}/versions/{version}`
- `POST /benchmark-suites/{suite_id}/versions/{version}/publish`
